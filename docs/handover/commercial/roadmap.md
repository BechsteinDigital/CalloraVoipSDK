...
Der WebRTC-/BUNDLE-/TURN-Track ist der zweite große Arbeitsstrang. Er ist überwiegend **funktional gebaut und gegen Fake-Server über Loopback getestet**, hat aber **keinen Browser- und keinen Echt-TURN-Server-Nachweis**. Das ist die zentrale Reife-Grenze und die Guardrail aus [ADR-009](../../adr/ADR-009-webrtc-browser-peer-roadmap.md): **kein „production-ready"-Claim für WebRTC ohne Browser-Interop-Validierung.**
...
- **Browser-Interop (SDK ↔ echter Browser)** — nicht gebaut ([ADR-009](../../adr/ADR-009-webrtc-browser-peer-roadmap.md) Guardrail).
- **Voll-E2E-Relay gegen echten TURN-Server (4d-6)** — nicht gebaut; jede Slice bisher nur gegen Fake-TURN.
...
|---|--------|----------------------|-------|
| 1 | **Browser-Interop-Validierung** | Gesamter WebRTC-/BUNDLE-/TURN-Track ohne verifizierten Lauf gegen Chrome/Firefox. Harness (echter Browser-Peer, Trickle-ICE, Candidate-Ordering) bauen. **Guardrail: bis dahin kein WebRTC-„production-ready".** | [ADR-009](../../adr/ADR-009-webrtc-browser-peer-roadmap.md); Risiko-Register §2 (Hoch); `AUDIT_HARDENING_BACKLOG` HARD-H8/B6 |
| 2 | **Echter-TURN-Server-E2E (4d-6)** | Kompletter Relay-Datenpfad nur gegen Fake-TURN über Loopback bewiesen; Wire-ChannelData-Roundtrip gegen echten `coturn`/TURN-Server aufsetzen. Voraussetzung für jeden NAT-durchquerenden Produktiveinsatz. | [ADR-054](../../adr/ADR-054-turn-relay-as-ice-candidate.md)/[ADR-055](../../adr/ADR-055-turn-control-stack-allocation-permission-keepalive.md)/[ADR-056](../../adr/ADR-056-post-nomination-whole-socket-relay-transition.md) (je „no real-server end-to-end run"); Risiko-Register §2 (Hoch) |
...
- **Die höchstwertige nächste Arbeit ist Validierung, nicht Neubau:** Browser-Interop (Posten 1) und echter TURN-E2E (Posten 2) heben vorhandenen Code auf Produktionsreife.
