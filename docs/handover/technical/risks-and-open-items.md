...
realen Referenz-Stack verifiziert". Der WebRTC-/TURN-/BUNDLE-Track ist überwiegend gegen Fake-/
In-Process-Server über Loopback getestet. **Zwei Reifepunkte sind seit 2026-07-27 belegt** (siehe
[`../../audit/INTEROP_SOAK_AUDIT.md`](../../audit/INTEROP_SOAK_AUDIT.md), Coverage-Notizen Paket 1+2):
(a) der **TURN-Relay-Datenpfad ist gegen einen echten in-Process-`TurnServer` über Loopback E2E-belegt**
(vier reale-Server-E2E-Tests: Allocation/Relay, Public-Relay-Address, Indication-Auth, Keepalive/Refresh
past Lifetime + ChannelData-Zustellung) — **offen** bleibt der Nachweis gegen einen **externen
Produktions-TURN-Server** (coturn); (b) **Browser-Audio-Interop ist erstmals bewiesen** (SDK-Offerer ↔
echter Chrome, Connect + bidir. Opus-Audio, lokal-first), und der dabei gefundene mDNS-`.local`-Blocker
ist SDK-seitig behoben. **WebRTC bleibt trotzdem nicht production-ready** — bewiesen ist nur Connect +
Audio in der SDK-Offerer-Richtung; Video-Browser-Interop, die Browser-Offerer-Richtung und die CI-Aufnahme
sind offen. Der SIP-/Audio-Kern ist dagegen gegen
**zwei echte SIP-Stacks** interop-getestet — **Asterisk** (im PR-CI-Gate) **und FreeSWITCH**
(lokal-first, gleiche `IPbxFixture`-Szenario-Matrix) — siehe
[ADR-058](../../adr/ADR-058-layered-test-interop-soak-model.md) und das Interop-/Soak-Register.
...
| Thema | Risiko / Offener Punkt | Schwere | Beleg | Empfohlene Maßnahme |
|-------|------------------------|---------|-------|---------------------|
| Browser-Audio-Interop bewiesen, aber nicht CI-gated | Der WebRTC-Kern ist erstmals gegen einen **echten Chrome** validiert: SDK-Offerer ↔ Browser-Answerer, volle Kette Signaling→ICE→DTLS→SRTP, **bidirektionales Opus-Audio** (`tests/CalloraVoipSdk.BrowserInteropTests`, Playwright/headless Chromium, 3× stabil lokal). Der dabei gefundene GA-Blocker (SDK droppte `.local`-mDNS-Candidates) ist behoben (`IMdnsResolver`/`SystemMdnsResolver`, RFC 8828). **Rest-Risiko:** der Test ist **lokal-first** (Kategorie `BrowserInterop`, aus allen CI-Jobs ausgeschlossen), also nicht CI-gated → keine Regressionsabsicherung im PR-Gate. | **Niedrig–Mittel** | [`../../audit/INTEROP_SOAK_AUDIT.md`](../../audit/INTEROP_SOAK_AUDIT.md) (Coverage-Notiz Paket 1); [ADR-009](../../adr/ADR-009-webrtc-browser-peer-roadmap.md); [ADR-060](../../adr/ADR-060-webrtc-facade-completion-and-server-hosting.md) | Browser-Test ins PR-CI-Gate aufnehmen (Playwright-Browser-Install-Step), damit der Interop-Nachweis vor Regression schützt. |
| Browser-Interop nur Audio + nur SDK-Offerer | Der bewiesene Umfang ist **Connect + bidir. Audio** in der **SDK-Offerer**-Richtung. **Video/VP8-Browser-Interop** und die **Browser-Offerer**-Richtung (Browser initiiert, SDK antwortet) sind **nicht** nachgewiesen. Loopback host-only (kein STUN/TURN im Browser-Call). | **Mittel** | [`../../audit/INTEROP_SOAK_AUDIT.md`](../../audit/INTEROP_SOAK_AUDIT.md) (Coverage-Notiz Paket 1, Scope-Grenze); [ADR-009](../../adr/ADR-009-webrtc-browser-peer-roadmap.md) | Folge-Slices: Video-Browser-Interop + Browser-Offerer-Richtung; erst dann WebRTC-Video/-Answerer-Pfad als browser-belegt führen. |
| WebRTC nicht production-ready (GA-Reifung läuft) | Trotz des Kern-Browser-Interop-Nachweises ist **kein „production-ready"/GA-Claim** für WebRTC zulässig: der bewiesene Umfang ist Connect + Audio (SDK-Offerer, lokal-first); es bleiben benannte GA-Blocker offen (Video-Browser-Interop, Browser-Offerer, CI-Gating, Answerer-Relay-Gap, TCP/TLS-TURN-Config-Diagnostik, Video-Stats-`null`). | **Mittel** | [`../../audit/INTEROP_SOAK_AUDIT.md`](../../audit/INTEROP_SOAK_AUDIT.md) (Coverage-Notizen Paket 1+2); [ADR-009](../../adr/ADR-009-webrtc-browser-peer-roadmap.md) (Guardrail) | GA-Reifung-Slices abarbeiten; WebRTC bis dahin als „Kern-Interop belegt, GA-Reifung offen" führen, nicht als produktionsreif. |
| TURN-Relay real-server-E2E belegt, aber nicht gegen externen coturn | Der TURN-Relay-Datenpfad (Allocation/Permission/ChannelBind/Keepalive/ChannelData) ist gegen einen **echten in-Process-`TurnServer` über Loopback** E2E-belegt (vier reale-Server-Tests: `TurnServerE2eTests`, `TurnPublicRelayAddressTests`, `TurnServerIndicationAuthE2eTests`, `TurnRelayKeepAliveE2eTests` inkl. Refresh past Lifetime + ChannelData-Zustellung, CF-003 „closes the only-fake-server-coverage gap"). **Rest-Risiko:** kein Nachweis gegen einen **externen Produktions-TURN-Server** (coturn); der Browser-E2E lief host-only **ohne** STUN/TURN → TURN-über-einen-Browser-Call ist **nicht** bewiesen. Ganze Session-Relay-Nominierung (Whole-Socket-Transition) bleibt timeout-bound (~10 s Direct-Exhaustion) → nicht im Fast-Unit-Test. | **Mittel** | [`../../audit/INTEROP_SOAK_AUDIT.md`](../../audit/INTEROP_SOAK_AUDIT.md); [ADR-054](../../adr/ADR-054-turn-relay-as-ice-candidate.md), [ADR-055](../../adr/ADR-055-turn-control-stack-allocation-permission-keepalive.md), [ADR-056](../../adr/ADR-056-post-nomination-whole-socket-relay-transition.md) | E2E gegen echten `coturn` aufsetzen + TURN-über-Browser-Call belegen (4d-6). Priorität für jeden NAT-durchquerenden Produktiveinsatz. |
| FreeSWITCH-Interop noch nicht im CI-Gate (lokal-first) | Der SIP-/Audio-Kern ist gegen **zwei** echte SIP-Stacks interop-belegt: **Asterisk** (`andrius/asterisk:22`, im PR-CI-Gate, 29 grün, 2-Bein-Media bidirektional) **und FreeSWITCH** (`safarov/freeswitch:latest`, gleiche `IPbxFixture`-Szenario-Matrix: Register/Media 7/7/DTMF/Hold/Transfer/Concurrent-Soak). Die FreeSWITCH-Suite ist **lokal-first** (Trait `Category=InteropFreeSwitch`) und **noch nicht ins PR-CI-Interop-Gate** aufgenommen (analog `SoakLong`) — CI-Gating ist ein Folge-Schritt, sobald über mehrere CI-Läufe stabil. | **Niedrig** | Interop-Register Coverage-Notiz FreeSWITCH (Phase B.2); [ADR-058](../../adr/ADR-058-layered-test-interop-soak-model.md); Memory-Anchor „Interop+Soak+Audit" | FreeSWITCH-Suite ins PR-CI-Interop-Gate aufnehmen (eigener Image-Pull-Step), sobald über mehrere Läufe stabil. |
| Weitere Referenz-Stacks (3CX/Fritzbox) offen | Über Asterisk + FreeSWITCH hinaus sind **3CX/Fritzbox** geplant, aber noch nicht als grüne Suite belegt. Zwei unabhängige Stacks (identischer Testcode grün) sind bereits ein starkes Konformitätssignal, aber nicht repräsentativ für alle Peer-Implementierungen. | **Niedrig–Mittel** | Interop-Register Coverage-Notizen; [ADR-058](../../adr/ADR-058-layered-test-interop-soak-model.md); Memory-Anchor „Interop+Soak+Audit" | Interop-Matrix auf weitere PBX/Endpunkte (3CX/Fritzbox) ausweiten. |
...
| CORE-011 Soak/Interop/Chaos-Gates (P0) | Die formalen Stabilitäts-Gates (Soak/Interop/Chaos als CI-Gate) sind das letzte offene GA-Bedingungs-Item (A+B sind ✅). Die Interop-Suite existiert (Asterisk im PR-CI-Gate, FreeSWITCH lokal-first), aber ein durchgängiges Chaos-/Soak-CI-Gate — inkl. FreeSWITCH-CI-Gating — ist noch nicht als geschlossen belegt. | **Mittel** | AUDIT_HARDENING_BACKLOG Paket I (CORE-011, P0); [ADR-058](../../adr/ADR-058-layered-test-interop-soak-model.md) | CORE-011 als letztes GA-Gate priorisieren. |

---

## Zusammenfassung für den Käufer

- **Reifer, interop-belegter Kern:** SIP-Signaling + Audio-Media (SDES/SRTP, DTLS-SRTP, RTCP, Codecs) sind gegen **zwei echte SIP-Stacks** end-to-end verifiziert — **Asterisk** (im PR-CI-Gate) **und FreeSWITCH** (lokal-first, gleiche Szenario-Matrix), inkl. bidirektionaler Zwei-Bein-Media; identischer Testcode auf zwei Herstellern = Konformitätssignal. Verbleibend: FreeSWITCH-CI-Gating und weitere Stacks (3CX/Fritzbox). Die kritischen Hardening- und Thread-Safety-Pakete (A, B, C, D, E, F) sind geschlossen.
- **Transport-vollständiger WebRTC-/TURN-/BUNDLE-Track mit erstem Interop-Nachweis, aber noch nicht GA:** funktional gebaut und unit-/loopback-getestet; der **TURN-Relay-Datenpfad ist gegen einen echten in-Process-`TurnServer` E2E-belegt** und der **WebRTC-Kern gegen einen echten Chrome (Connect + bidir. Audio, lokal-first)** — inkl. behobenem mDNS-`.local`-Blocker. **Weiterhin nicht production-ready:** offen bleiben Video-Browser-Interop, die Browser-Offerer-Richtung, die CI-Aufnahme des Browser-Tests, der Answerer-Relay-Gap, TCP/TLS-TURN und ein TURN-Nachweis gegen einen externen coturn. Kein „production-ready"/GA-Claim für WebRTC zulässig, bis diese Rest-Slices existieren.
