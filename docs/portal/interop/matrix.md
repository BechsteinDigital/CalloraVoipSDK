...
| **coturn** (UDP TURN relay) | ✅ Automated in CI | TURN relay allocation and media over a real coturn server, end-to-end |
| **TCP/TLS TURN relay** (stream relay, ADR-073) | 🧪 Unit / hosted-server proven; browser & cloud-TURN pending | New in 4.11 (RFC 8656 §12 ChannelData over a persistent TCP/TLS connection). Verified against a hosted `TurnServer` over **both TCP and TLS** — allocate, gather a relay candidate, install a permission, frame a Send indication — plus the media ChannelData send→inject→decrypt round-trip and a nominated-relay media switch, in unit/integration tests on every CI run. **Not yet** exercised over a real browser or a real cloud TURN server; that data-path interop is tracked in #228 / #241 |
