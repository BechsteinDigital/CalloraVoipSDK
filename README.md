...
Modules build on the per-call media tap, and its contract is two sentences worth reading twice:
...
**Commercial modules** are in development on top of this seam, distributed through a private feed
rather than nuget.org: **Callora.Realtime** (call audio to realtime AI APIs, with pacing, backpressure
and barge-in — the basis for voice agents), **Callora.WebSocket** (raw call audio over a socket), and
**Callora.Privacy / Risk / Intelligence** (redaction and consent, spam screening, transcription and
sentiment). The SDK core stays open and free; the modules are licensed separately —
