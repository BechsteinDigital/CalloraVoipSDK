...
public sealed class VideoLossReportTests
{
    ...
    public void In_order_arrival_reports_no_loss()
        => Assert.Null(VideoArrivalLossTracker.LossReport(100, 101));

    [Fact]
    public void Duplicate_arrival_reports_no_loss()
        => Assert.Null(VideoArrivalLossTracker.LossReport(100, 100));

    [Fact]
    public void Backward_reorder_reports_no_loss()
        => Assert.Null(VideoArrivalLossTracker.LossReport(100, 98));

    [Fact]
    public void Far_backward_reorder_reports_no_loss()
        => Assert.Null(VideoArrivalLossTracker.LossReport(100, 40000));

    [Fact]
    public void Small_forward_gap_names_the_missing_sequence_numbers()
        => Assert.Equal((ushort[])[101, 102], VideoArrivalLossTracker.LossReport(100, 103));

    [Fact]
    public void Single_missing_packet_is_named()
        => Assert.Equal((ushort[])[101], VideoArrivalLossTracker.LossReport(100, 102));

    [Fact]
    public void Forward_gap_at_the_enumeration_boundary_is_still_named()
    {
        var report = VideoArrivalLossTracker.LossReport(100, (ushort)(100 + 256));
    ...
    public void Forward_loss_beyond_the_enumeration_boundary_reports_pli_only()
    {
        ...
        var report = VideoArrivalLossTracker.LossReport(100, (ushort)(100 + 257));
        Assert.NotNull(report);
        Assert.Empty(report!);
    }
    ...
    public void Forward_gap_across_the_wrap_boundary_is_named()
        ...
        => Assert.Equal((ushort[])[0], VideoArrivalLossTracker.LossReport(65535, 1));
    ...
    public void Reorder_across_the_wrap_boundary_reports_no_loss()
        ...
        => Assert.Null(VideoArrivalLossTracker.LossReport(1, 65535));
}
